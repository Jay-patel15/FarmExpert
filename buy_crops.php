				...
				<form role="form"  id="buycrops"  method="POST" enctype="multipart/form-data">   
                    ...
                    <tr>
						...
						if(isset($_POST['calculate'])){
							...
							// Check if crop exists in farmer_crops_trade
							$query1 = "SELECT Trade_crop FROM farmer_crops_trade WHERE Trade_crop='$crop'";
							...
							if(mysqli_num_rows($result1) > 0) {
								...
								$query2 = "SELECT SUM(Crop_quantity) AS total_quantity FROM farmer_crops_trade WHERE Trade_crop='$crop'";
								$result2 = mysqli_query($conn, $query2);
								$row2 = mysqli_fetch_assoc($result2);
								$temp = isset($row2['total_quantity']) ? $row2['total_quantity'] : 0;
						
								// Get quantity from cart
								$query8 = "SELECT quantity FROM cart WHERE cropname='$crop'";
								$result8 = mysqli_query($conn, $query8);
								$row8 = mysqli_fetch_assoc($result8);
								$temp -= isset($row8['quantity']) ? $row8['quantity'] : 0;
						
								if($quantity > $temp) {
									$flag = 0; // Not enough stock available
								}
						
								// Get price if the crop is available
								...
								// Get trade_id if crop is available
								$query3 = "SELECT trade_id FROM farmer_crops_trade WHERE Trade_crop='$crop' LIMIT 1";
								...
								echo "<script>alert('Selected crop is not available in the database');</script>";
							}
						}
